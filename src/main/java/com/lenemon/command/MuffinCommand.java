...
public final class MuffinCommand {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        var typeArgument = CommandManager.argument("type", StringArgumentType.word())
        ...
        dispatcher.register(CommandManager.literal("lenemon")
    ...
    private static int executeGive(ServerCommandSource source, ServerPlayerEntity target, String rawType, int quantity) {
        ...
        while (remaining > 0) {
            ...
            ItemStack stack = MagicMuffinHelper.create(type);
            stack.setCount(batch);
            target.giveItemStack(stack);
            remaining -= batch;
        }

        source.sendFeedback(() -> Text.literal("§a[Muffin] " + quantity + " muffin(s) " + type.id() + " donne(s) a §f" + target.getName().getString()), false);
        if (!source.getName().equalsIgnoreCase(target.getName().getString())) {
            target.sendMessage(Text.literal("§6[Muffin] Vous avez recu §e" + quantity + "§6 " + type.displayName() + "§6."), false);
        }
        return 1;
    }
